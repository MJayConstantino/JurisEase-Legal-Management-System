...
interface MatterBillAddDialogProps {
  open: boolean
...
export function MatterBillAddDialog({
  open,
  onOpenChange,
  onSave,
  currentMatterId,
}: MatterBillAddDialogProps) {

  const { name, setName, remarks, setRemarks, amount,
    setAmount, created_at, setCreated_at, status, setStatus
  } = BillingStates()
  const [matter_id, setMatterId] = useState(currentMatterId)
