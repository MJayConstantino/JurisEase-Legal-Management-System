describe('Global Search E2E', () => {
  ...
  describe.skip('When a user wants to search for a task', () => {
    ...
    it('shoould display relevant tasks whose name (including those without matters) matches the search query if the tasks checkbox is checked and no search by checkboxes are checked', () => {
      cy.get('#matters').click()
      cy.get('input[aria-label="DialogBoxSearch"]').type('Mock Task')
      cy.wait(2000)
        .contains(/Mock Task (No Matter)/i)
        .should('be.visible')
      cy.contains(/Mock Task (With Matter)/i).should('be.visible')
    })
